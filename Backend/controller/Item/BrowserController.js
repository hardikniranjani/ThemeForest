...
router.put('/active/:id', async (req, res) => {
    ...
    const result = await Browser_Model.findOne({ _id: id,isActive: false});
    if (!result) return res.status(404).send({ Message: 'Browsers not found' });
    ...
    if(!updatedResult) return res.status(500).send({ Message: "Can't active browser right now" });

    return res.status(200).send({ Message: "Browser active successfully" });
